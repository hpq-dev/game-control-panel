$(document).ready(function() {
	...
	$(document).on('click', '#save-notice', function() {
		...
		let i = $(this).data('id');
		$.ajax({url:URL+"api/update-comment/complaint", type:"POST", data:{Text: $('#notice-data-' + i).val(), ID: i},
            success:function(result){
	...
	$(document).on('click', '#delete-text', function() {
		let i = $(this).data('id');
		$.ajax({url:URL+"api/update-comment/complaint/delete", type:"POST", data:{ID: i},
            success:function(result){
	           	$('#main-media-' + i).remove();
