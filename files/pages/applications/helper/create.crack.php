...
if($player_data->Helper||$player_data->Admin) {
    echo Config::afterShowInfo('', 'Nu poti aplica la functia de helper (Esti ' . ($player_data->Admin ? 'Admin' : 'Helper') . ') !', 'error');
    exit;
}
...
if(Token::Check()) {
    if(isset($_POST['app_submit'])) {
        ...
        if(!$field) {
            ...
            $query->execute(array($player_data->name, $_SESSION['LOGGED'], 1, $code, date("d.m.Y H:i")));
